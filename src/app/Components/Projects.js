...
import Skills from "./Skills"; // Ensure Skills contains a 'Projects' array

const Projects = () => {
  return (
    <div className="flex justify-center items-center pt-10">
      <div className="grid md:grid-cols-2 gap-20">
        {Skills.Projects.map((data, index) => (
          <div
            key={index}
            className="w-full h-full p-5 text-white bg-amber-600/10 border-2 border-transparent rounded-lg transform origin-bottom-right transition-all duration-500 ease-out hover:rotate-3 hover:border-orange-500 hover:shadow-lg flex flex-col"
          >
            <div className="flex flex-col flex-grow">
              {/* Header */}

              {/* Title */}
              <p className="text-2xl font-semibold my-6">{data.projectname}</p>
              ...
              <div className="flex flex-wrap gap-2">
                {data.technologies.map((tech, key) => (
                  <span
                    key={key}
                    className="bg-orange-500 px-3 py-1 text-xs font-bold uppercase rounded-full"
                  >
                    {tech}
                  </span>
                ))}
